---
name: tailor-resume
description: Tailor Rukaiya's resume to job notes from Obsidian (Jobs/*.md), export one-page PDFs, and write match reports back. Also tailors all pending jobs at once, marks jobs applied (with follow-up dates), and writes the weekly job-search review. Use for any request about tailoring resumes, job notes, applications, follow-ups or the weekly review.
...
The same workflows are MCP prompts too: `/mcp__resume-ai__tailor_resume`, `/mcp__resume-ai__tailor_all_pending`, `/mcp__resume-ai__weekly_review`.
