...
export function ResumeDocument({ resume, design, mode = 'screen', selectedSectionId, onSectionClick, onFit, links = mode === 'print' }: ResumeDocumentProps) {
  ...
  const scale = useFitScale(contentRef, design, JSON.stringify([resume.sections, design]), onFit);
  const d = design;
  ...
  return (
    <PageFrame design={d} mode={mode}>
      <div ref={contentRef} style={contentStyle(d, scale)} data-resume-content>
        {basics?.basics && basics.visible !== false && (
          <SectionFrame section={basics} selected={selectedSectionId === basics.id} onClick={onSectionClick} style={{}}>
            <DocumentHeader basics={basics.basics} design={d} links={links} />
          </SectionFrame>
        )}
...
/** Name + contact line, shared by resumes and cover letters so they match. */
export function DocumentHeader({ basics, design: d, links }: { basics: Basics; design: Design; links: boolean }) {
